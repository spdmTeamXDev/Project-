		...
		then
			...
			if recordReturnValues and (methodName == "InvokeServer" or methodName == "invokeServer") then
				...
				task.defer(function()
					...
					coroutine.wrap(function()
						schedule(remoteHandler, True, methodName, remote, args, funcInfo, calling, returnValue)
					end)()
				end)
			else
				coroutine.wrap(function()
					schedule(remoteHandler, True, methodName, remote, args, funcInfo, calling)
				end)()
			end
		end
		if recordReturnValues and (methodName == "InvokeServer" or methodName == "invokeServer") then
...
if not _G.SimpleSpyExecuted then
	local succeeded, err = pcall(function()
		...
		-- if gethui then funcEnabled = True end
		onToggleButtonClick()
